...
Pois, a abordagem tradicional ao problema seria o desenvolvimento de uma \acr{API} para receber e permitir a consulta dos dados.
Entretanto, por disponibilizar os dados dos usuários do framework na Internet haveria a necessidade de criar uma camada de autorização na \acr{API}, ambas tarefas que expandem consideravelmente o escopo de desenvolvimento.
A solução encontrada para esse problema é a utilização do programa de planilhas online Google Sheets, onde ao invés da execução do framework enviar as métricas para uma \acr{API} nossa, ela envia para uma planilha do desenvolvedor.
