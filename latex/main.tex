...
\label{cap:metodologiatrabsrelacionados}
	...
	\begin{table}[ht]
		...
		A ferramenta é orientada à medição de websites e páginas na web, e interaje com essas páginas via requisições do tipo \acr{HTTP}. Dessa forma, atende ao critério de Coesão e Acoplamento, pois não exige que o código fonte seja modificado para o seu uso, mas não atende ao critério de Portabilidade, pois é utilizada somente para o contexto de websites.

		Como a ferramenta realiza a persistência de dados de forma remota, atende ao critério de Armazenamento em Núvem. Em contrapartida, a ferramenta é capaz de lidar somente com métricas de \acr{QoS}, não sendo possível customizar métricas, e por isso não atende ao critério de Extensibilidade.
...
Considere uma pesquisa onde são comparadas diferentes métodos para gerar sequências de números primos, e deseja-se de forma empírica obter qual é a melhor abordagem. Para isso, é necessário gerar essas sequências utilizando esses diferentes métodos e persistir esses resultados, afim de analisar qual tem o melhor comportamento. Porém, existe uma variável nesse processo que é o ambiente no qual estão sendo executados esses métodos. Nesse caso o \acr{ELC} pode ajudar, pois habilita que esse experimento seja feito de forma remota por voluntários, que podem ter diversos tipos de ambientes. 

Para realizar esse experimento utilizando o \acr{ELC}, primeiro é necessário criar um projeto para o experimento, como é mostrado na figura XX. No projeto, sequencialmente, são executadas as Tarefas, e após a execução de cada uma, persistidos os dados em uma tabela no Google Sheets. No contexto do exemplo, para cada execução serão persistidos N registros, onde N é o número de métodos diferentes de se gerar números primos a serem avaliados.
...
Para realizar a coleta de métricas dentro do site com o \acr{ELC}, o primeiro passo é realizar a instalação do pacote el-chupacabra por meio do \acr{NPM}\footnote{https://www.npmjs.com/package/el-chupacabra}.
...
Para o uso simplificado do \acr{ELC}, foi criado um módulo que contém somente o básico para executar um experimento com a ferramenta. Esse módulo foi disponibilizado via \acr{NPM}, e pode ser instalado em qualquer projeto Javascript. Esse módulo é composto de duas classes principais, a EasyElc e a Profiling.
...
% TO-DO: Falar sobre o experimento mais completo, usando o ELC clonando direto do repositório.
