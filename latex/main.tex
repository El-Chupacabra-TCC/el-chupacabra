...
Para a métrica de memória, foi criado um objeto chamado Resource, que simula a memória total, e os métodos alloc e free para simular a alocação e desalocação de memória, e um metodo total para retornar o valor atual da memória.
...
\end{lstlisting}
Foi utilizado métodos estáticos para não ser preciso instanciar e passar como parametro essa classe.
...
\end{lstlisting}
Para implementar novas metricas basta criar uma classe que herde da classe Metric e implementar os métodos abstratos.
...
O metodo chamado run, é responsavel por executar estes outros métodos e retornar as metricas. Segue a implementação da classe abstrata de tarefa:
\begin{lstlisting}[language=TypeScript]
abstract class Task {
	public name: string;
