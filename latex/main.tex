...
Neste capítulo estão as descrições do experimento utilizado para avaliar o \acr{ELC} por volutários externos, bem como sua preparação e execução. Além disso, estão contidos também os resultados do experimento, e também a análise parcial dos resultados.

\section{Preparação do experimento}
\label{section:preparacao-experimento}

Para o experimento, foram definidas duas etapas. A primeira é a utilização do EasyELC em um projeto exemplo, na plataforma \emph{Codepen}\footnote{Plataforma Codepen: https://codepen.io/}. A segunda foi o preenchimento de um formulário no Google Forms, com o fim de coletar a avaliação do voluntário de como foi a experiência de uso do \acr{ELC}. 
Para o grupo de controle, foram consideradas XX pessoas,entre alunos e professores. 

\section{Uso do ELC pelos voluntários}
\label{section:parte-1-experimento}

O experimento consiste basicamente...

Baixar um JSON no código Javascript
O JSON vai ficar em uma variável 
E será ordenado usando: heapsort e bubblesort
O desafio do usuário vai ser descobrir qual o consumo de memória e quanto tempo ele leva pra ordenar
O ELC vai estar pré configurado para subir os dados em uma planilha do google sheets
Com isso ele vai adicionar o que precisa no código javascript pra coleta acontecer
E depois preencher o formulário

\section{Formulário}
\label{section:parte-2-formulario}

Na Tabela \ref{tab:perguntas-experimentos} estão descritas as perguntas utilizadas no formulário enviado para ser preenchido após o experimento.

\begin{table}[h!]
\centering
\caption{Perguntas utilizadas no formulário de avaliação}
	\begin{tabular}{c} 
	\hline
		Pergunta 1 \\
		Pergunta 2 \\
	...
	\hline
	\end{tabular}
	\label{tab:perguntas-experimentos}
\end{table}

\section{Resultados}
\label{section:resultados}

%Parágrafo sobre o resultado no codepen. Se alguém teve dificuldade, etc.

Após a submissão dos formulários, foi possível gerar um gráfico com as respostas para cada pergunta do experimento, descritos abaixo.
...
\par Após o desenvolvimento da ferramenta, é possível posicioná-la com relação aos critérios que foram estabelecidos na seção \ref{sec:avaliacao-ferramentas-encontradas}. O resultado da comparação com as outras ferramentas se encontra na tabela \ref{table:classificacao-com-elc}.
...
Inclusão de métricas de caixa branca
