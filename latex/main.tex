...
\begin{itemize}
	\item Perfil do ambiente de execução: As avaliações usando o \acr{ELC} podem ser executadas em diferentes tipos ambientes. Para que seja possível coletar informações sobre o ambiente que a avaliação está sendo executada é necessário utilizar uma instancia de perfil de execução adequada. Para este exemplo, consideramos a execução em navegadores.
	\item Persistência das métricas: Onde os dados coletados pelo \acr{ELC} são armazedados. Para este exemplo, consideramos que as métricas coletadas devem ser enviadas para uma planilha no Google Sheets.
	\item Árvore de tarefas: Tarefa é nome das classes que são a carga de trabalho sob avaliação do \acr{ELC}. Porém, a estrutura utilizada para sua execução é de árvore, deste modo, somos capazes de subdividir análises.
...
Se o artefato for gerado com tudo padrão do repositório, ele será um site estático com um botão para executar a avaliação.
