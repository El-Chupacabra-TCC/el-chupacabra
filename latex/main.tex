...
Porém, com o intuito de tornar isso possível, além da prova de conceito, a equipe ainda precisaria desenvolver como expor, executar e coletar as métricas durante a avaliação. Este é o tipo de cenário para qual o ELC (TO-DO: Definir a sigla - incluir nota de rodapé) está sendo proposto. 

{{TO-DO: INSERIR FIGURA DO ELC. VISÃO ARQUITETURAL. UMA VISÃO SIMPLIFICADA DO QUE O ELC FAZ}}

A contribuição principal do ELC é oferecer uma forma fácil e eficaz de realizar a execução de tarefas customizáveis no lado do cliente, e também o registro dos resultados dessas execuções em uma base de dados, também customizável. Dessa forma, seria possível de forma simples resolver o problema da equipe de desenvolvimento. - TO-DO: O que seria fácil? (Pouca configuração, sem mexer muito no código do cliente, integração rápida sem a necessidade de refatorar a aplicação.) - Lembrar isso na conclusão.

TO-DO: Limitação: métricas de caixa branca são uma limitação. Só são coletadas métricas em tempo de execução. Métricas visuais: os outros frameworks abordam, mas o ELC não.
...
\subsection{Uso do chupacabra em ambiente produtivo}

Considere o caso de uma aplicação que já está em ambiente de produção com fluxo ativo de usuários.
E, estes usuários contantemente reclamam sobre lentidões no site.
Portanto, algum desenvolvedor é designado a investigar as causas dessas lentidões.
Neste cenário, o desenvolvedor geralmente realiza tentativas de reproduzir o problema do cliente em sua máquina.
Entretanto, o ideal seria que ele tivesse os dados das execuções dos usuários e para isso o \acr{ELC} oferece o módulo EasyElc.
...
Onde o usuário insere a quantidade de números primos que deseja e o site calcula a sequência dos números primos, do 2 até a quantidade solicitada.
...
% TO-DO: Adicionar imagem do site após realizar o cálculo de 10 números primos com as configurações abertas.
...
Com o \acr{ELC} instalado como depêndencia do seu projeto, podemos dentro da base de código do site criar uma instância do EasyElc.
E, como o objetivo é obter métricas dos usuários de um site, são utilizadas como configurações:
O perfilador de execução para navegadores e a coleta de métricas por meio do Google Sheets.
...
Com uma instância do EasyElc criada, já somos capazes de realizar a coleta de métricas.
A qual é realizada por meio da chamada do método startProfiling que recebe 
um nome do que está sendo medido e uma lista de métricas para coletar.
% TO-DO: Adicionar referência do livro do GOF no Strategy.
Como a implementação do site faz uso do padrão de projeto Strategy, tudo que é preciso é envolver a chamada do método getFirstNPrimeNumbers.
Então, com a chamada do método finish finalizamos aquela coleta de métricas, passando metadados como argumento, e os dados coletados são enviados para a planilha.
