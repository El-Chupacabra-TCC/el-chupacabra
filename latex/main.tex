...
const easyElc = new EasyElc(
  ...
  new Persisters.SheetsonPersister(
	...
	Por último, quando todas as análise são finalizadas, o \acr{ELC} realiza a persistência dos dados.
...
\end{minipage}
	...
	Por outro lado, a implementação dos módulos fornecem uma visão do que é possível realizar usando o \acr{ELC}.
	...
	O Diagrama de Classes é um elemento importantíssimo na construção de uma ferramenta. Nele há a descrição das classes, dos elementos que as compõem e dos seus relacionamentos, carregando assim a definição dos principais aspectos práticos para a implementação do sistema. Por exemplo, são definidas as árvores de herança e principais métodos que precisam ser implementados pelo usuário. Dessa forma, o Diagrama de Classes do \acr{ELC} se encontra na figura \ref{fig:diag-classes}, e após o diagrama estão as descrições de cada um dos seus elementos.
	...
	\begin{description}
		...
		\label{desc:descricao-diagrama-classes}
	\end{description}
	...
	A arquitetura do \acr{ELC} tem como guia fundamental o princípio aberto/fechado (\acr{OCP}), o qual é detalhado na seção \ref{sec:solid-ocp}.
	...
	Por outro lado, os outros módulos compõem o núclo do \acr{ELC}, sendo eles: O módulo de perfil de execução (do inglês, \emph{ExecutionProfile}) para obteção de dados sobre o ambiente o qual está sendo executado, o módulo de persistência (do inglês, \emph{Persister}) para encapsular o meios de armazenar os resultados das análises, o módulo de métricas (do inglês, \emph{Metrics}) que corresponde as medições o qual o \acr{ELC} é capaz de realizar e o módulo de tarefas (do inglês, \emph{Tasks}) encapsula os procedimentos em análise pela ferramenta.
...
\end{lstlisting}
	...
	Ademais, com informações suficiente é possível criar um identificador único para cada ambiente executando o \acr{ELC}, do mesmo modo que um programa de fingerprinting.
	
	Todos os módulos do núcleo do \acr{ELC} possuem interfaces as quais são o único elemento que pode ser refenciado por outros módulos.
...
\end{lstlisting}
	...
	Ele utiliza uma \acr{API} gratuita denominada Sheetson para persistir os dado em uma planilha do Google Sheets e sua configuração de uso é simples, como pode ser visto na listagem \ref{lst:elc_index_ts_example}.
...
\chapter{Experimentos}
	...
	\begin{lstlisting}[label={lst:metrica-observable}, caption={Seção para criação de métrica customizada pelo usuário no experimento.}, language=TypeScript, breaklines=true]
		class myCustomRngMetric {
			// Você não precisa alterar este método
			start() {
			}
	...
	\begin{lstlisting}[label={lst:experimento-heap}, caption={Fluxo de código referente a execução da ordenação com o Heap Sort.}, language=TypeScript, breaklines=true]
		async function heapSortEvaluation(airports) {
			...
			const heapProfiling = ezElc.startProfiling(
				"Heap Sort",
				// Adicione no array abaixo a sua métrica customizada
				[
				new elChupacabra.Metrics.DeltaTimeMetric()
				]
			);
...
\chapter{Conclusão}
	...
	Além disso, juntamente com as métricas, é possível registrar informações sobre o dispositivo e software que executaram a avaliação, atendendo à experimentos multiplataforma que precisam destas informações. Portanto, é possível dizer que a ferramenta atende ao critério de Portabilidade.

	No armazenamento dos resultados, o \acr{ELC} também atende aos requisitos extensibilidade.
