...
\dresumo{
Desde a popularização dos smartphones a quantidade de dados gerados cresce sem precedentes, de modo que até torna a definição de grande volume de dados difícil por uma rápida obsolescência.
Dentro deste contexto, aplicações Web as quais ofertam visualização desses dados sofrem muitas vezes com a limitação de recursos existente no lado do cliente e seus desenvolvedores possuem dificuldades de avaliar o consumo dos recursos computacionais cobrindo ampla gama de configurações.
Além disso, há uma carência por ferramentas as quais permitam esse tipo de avaliação e que facilitem o trabalho de cobrir uma grande variedade de tipos de clientes, por exemplo, contando com o auxílio de voluntários.
A fim de solucionar estes problemas, propomos um framework para o ecossistema JavaScript capaz de executar provas de conceito em paralelo, realizando a coleta de métricas de desempenho e consumo de recursos computacionais a qual também seja facilmente extensível e executável por voluntários.
}{big data; Web; visualização de dados; avaliação de desempenho; consumo de recursos; framework}	

\dabstract{
Since the popularization of smartphones, the amount of data generated has grown unprecedentedly, so that even the definition of how big is big data becomes difficult due to rapid obsolescence.
Within this context, Web applications which offer visualization of big data often suffer from the limitation of resources on the client side and their developers have difficulties in evaluating the consumption of computational resources covering a wide range of setups.
In addition, there is a lack of tools that allow this type of assessment and that facilitate the work of covering a wide variety of types of customers, for example, with the help of volunteers.
In order to solve these problems, we propose a framework for the JavaScript ecosystem capable of running proofs of concept in parallel, collecting performance metrics and consumption of computational resources, which is also easily extensible and executable by volunteers.
