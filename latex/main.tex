...
\subsection{Mock de recursos}
No desenvolvimento da prova de conceito foram criados recursos fictícios de memória e de tempo de processamento.
Para a métrica de memória, foi criado um objeto chamado Resource, que simula a memória total, e os métodos alloc e free para simular a alocação e desalocação de memória, e um método total para retornar o valor atual da memória.
...
\end{lstlisting}
Foi utilizado métodos estáticos para não ser preciso instanciar e passar como parâmetro essa classe.

Para tempo de processamento foi criado um método chamado sleep, que simula o tempo de processamento de uma tarefa.
\begin{lstlisting}[language=TypeScript]
class MockTime {
	public static sleep(
		min: number,
...
\end{lstlisting}
Para usar este metodo basta clamar ele passando uma janela de tempo mínima e máxima, e o tempo esperado será algo entre esses valores.

\subsection{Metricas}
A partir da simulação do uso de recursos de memória e processamento, foi possível gerar métricas viáveis e avaliar os seus resultados. Foram implementadas 2 tipos de métricas, a métrica de memória e a de tempo.
...
\end{lstlisting}
Já na métrica de tempo de processamento, é verificado o tempo que a tarefa ficou em execução. Segue o exemplo:
\begin{lstlisting}[language=TypeScript]
...
\end{lstlisting}
Para implementar novas métricas basta criar uma classe que herde da classe Metric e implementar os métodos abstratos.
