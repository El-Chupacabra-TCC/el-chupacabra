...
\lstdefinelanguage{TypeScript}{
    keywords=[1]{import, from, export, static, public, implements, extends, class, extends, private, void, constructor, super, this, async, await, for, of, let, in, return, console, log},
    keywordstyle=[1]\color{blue},
    keywords=[3]{MockTime, Promise, IPersister, Task, CompositeTask, BasicPersister, Resource, Project, MockTask, Metric, DeltaTimeMetric, ProcessMemoryMetric, Set, JSON, Math, any},
...
\subsection{Mock re recursos}
No desenvolvimento da prova de conceito foram criados recursos fictícios de memória e de tempo de processamento.
Para a métrica de memória, foi criado um objeto chamado Resource, que simula a memória total, e os metodos alloc e free para simular a alocação e desalocação de memória, e um metodo total para retornar o valor atual da memória.
...
\end{lstlisting}
Foi utilizado metodos estáticos para não ser preciso instanciar e passar como parametro essa classe.

Para tempo de provessamento foi criado um metodo chamado sleep, que simula o tempo de processamento de uma tarefa.
\begin{lstlisting}[language=TypeScript]
class MockTime {
	public static sleep(
		min: number,
...
\end{lstlisting}
Para usar este metodo basta clamar ele passando uma janela de tempo minima e maxima. e o tempo esperado será algo entre esses valores.

\subsection{Metricas}
A partir da simulação do uso de recursos de memória e processamento, foi possível gerar métricas viáveis e avaliar os seus resultados. Foram implementadas 2 tipose de metricas, a metrica de memoria e a de tempo.
Na métrica de memória, é verificado o quanto da memória simulada foi alocada.
Segue o exemplo de implementação:
\begin{lstlisting}[language=TypeScript]
class GetDeltaTime extends Metric<number> {
  private date: number;
  constructor() {
    super();
    this.date = Date.now();
...
\end{lstlisting}
Já a métrica de tempo de processamento, é verificado o tempo que a tarefa ficou em execução. Segue o exemplo:
\begin{lstlisting}[language=TypeScript]
...
\end{lstlisting}
Para implementar novas metricas basta criar uma classe que herde da classe Metric e implementar os metodos abstratos.
...
Já as tarefas compostas são uma agregação de tarefas simples ou compostas que também tem suas métricas, ela tem um metodo chamado addTask, que é responsavel por adicionar uma tarefa a lista de tarefas da tarefa composta. Isso permite a criação de árvores de tarefas para serem avaliadas, dando liberdade ao usuário.

Para a execução ambos tipos de tarefa herdam da classe de tarefa abstrata, essa classe é responsavel por executar a tarefa e retornar as metricas.
Ela é composta por 3 etapas, a primeira é a preTaskJob, que é responsavel por executar as tarefas que devem ser executadas antes da tarefa principal, a segunda é a execução da tarefa principal, e a terceira é a postTaskJob, que é responsavel por executar as tarefas que devem ser executadas depois da tarefa principal.
Essas etapas são executadas para todas as tarefas.
O metodo chamado run, é responsavel por executar estes outros metodos e retornar as metricas. Segue a implementação da classe abstrata de tarefa:
\begin{lstlisting}[language=TypeScript]
abstract class Task {
	public name: string;
...
Na tarefa composta o metodo execute é responsavel por executar todas as tarefas da lista de tarefas da tarefa composta e retornar as metricas. Segue a implementação:
\begin{lstlisting}[language=TypeScript]
